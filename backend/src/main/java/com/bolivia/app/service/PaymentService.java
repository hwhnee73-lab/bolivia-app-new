...
public class PaymentService {
    ...
    @Transactional(readOnly = true)
    public java.util.List<com.bolivia.app.dto.BillDto.PaymentInfo> getPaymentHistory(Long userId) {
        String sql = "SELECT id, bill_id, amount, payment_method, payment_date FROM payments WHERE user_id = ? ORDER BY payment_date DESC";
        return jdbcTemplate.query(sql, (rs, rn) -> new com.bolivia.app.dto.BillDto.PaymentInfo(
                rs.getLong("id"),
                rs.getLong("bill_id"),
                rs.getBigDecimal("amount"),
