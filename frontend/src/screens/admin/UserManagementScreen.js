...
const UserManagementScreen = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showToast(
        editingUser ? t("users.success.updated") : t("users.success.created"),
      );
  ...
  const statusColor = {
    ...
    LOCKED: "text-red-800 bg-red-100",
  };
    ...
    <div className="bg-white rounded-lg shadow-lg p-6 md:p-8 text-gray-800">
      ...
      {isFormVisible && (
        <form
          ...
          className="bg-gray-50 p-4 rounded-lg mb-6 space-y-4 border"
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <input
              ...
              placeholder="Nombre (Ej: Juan Residente)"
            ...
            <input
              ...
              value={formData.dong}
              onChange={handleInputChange}
              placeholder="Torre (Ej: 0000000101)"
              className="w-full p-2 border rounded"
              required
            />
            <input
              ...
              value={formData.ho}
              onChange={handleInputChange}
              placeholder="Apto (Ej: 0000001502)"
              className="w-full p-2 border rounded"
              required
            />
            <select
          ...
          <div className="flex gap-2 pt-2">
            ...
            >
              {editingUser ? t("common.saveChanges") : t("common.register")}
            </button>
            <button
              type="button"
        ...
        <table className="w-full text-sm text-left">
          <thead className="bg-gray-100">
            <tr>
              ...
              <th className="p-3">{t("users.table.sessionId")}</th>
          ...
          <tbody>
            {users.map((user) => (
              <tr key={user.id} className="border-b hover:bg-gray-50">
                ...
                <td className="p-3">
                  <span
                    className={`px-2 py-1 text-xs rounded-full font-medium ${statusColor[user.status]}`}
                  >
                    {user.status}
                  </span>
                </td>
