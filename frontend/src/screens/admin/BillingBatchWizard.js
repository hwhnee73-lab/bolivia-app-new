...
const Step = ({ n, current, label }) => (
  <div
    className={`flex-1 text-center py-2 rounded ${current === n ? "bg-teal-600 text-white" : "bg-gray-100 text-gray-700"}`}
  >
    {n}. {label}
  </div>
);
...
const BillingBatchWizard = () => {
  ...
  const upload = async (e) => {
    e.preventDefault();
    if (!file) {
      setError(t("billing.errors.selectFile"));
      return;
    }
    setLoading(true);
    setError("");
  ...
  const confirm = async () => {
    if (!preview?.tokenKey) {
      setError(t("billing.errors.missingToken"));
      return;
    }
    setLoading(true);
    setError("");
      ...
      {step === 1 && (
        <form onSubmit={upload} className="space-y-4">
          ...
          <div className="flex gap-2">
            ...
            >
              {t("billing.actions.downloadSample")}
            </button>
          </div>
        </form>
      )}
        ...
        <div className="mt-4">
          <div className="flex flex-wrap gap-3 mb-3 text-sm">
            <span className="px-2 py-1 bg-gray-100 rounded">
              {t("billing.preview.totalRows", { count: preview.totalRows })}
            </span>
            <span className="px-2 py-1 bg-green-100 text-green-700 rounded">
              {t("billing.preview.valid", { count: preview.validCount })}
            </span>
            <span className="px-2 py-1 bg-red-100 text-red-700 rounded">
              {t("billing.preview.invalid", { count: preview.invalidCount })}
            </span>
            <span className="px-2 py-1 bg-gray-100 rounded">
              {t("billing.preview.token", { token: preview.tokenKey })}
            </span>
          </div>
              ...
              <tbody>
                {preview.rows.map((r, idx) => (
                  <tr key={idx} className="border-t">
                    ...
                    <td className="p-2">
                      {r.valid ? (
                        <span className="text-green-600">{t("common.ok")}</span>
                      ) : (
                        <span className="text-red-600">
                          {r.error || t("common.errorShort")}
                        </span>
                      )}
                    </td>
                  </tr>
        ...
        <div className="mt-6">
          <div className="p-4 bg-green-50 text-green-800 rounded">
            ...
            <p className="mt-1 text-sm">
              {t("billing.result.summary", {
                total: confirmResult.total,
                upserted: confirmResult.upserted,
                skipped: confirmResult.skippedInvalid,
              })}
            </p>
          </div>
          <div className="mt-4">
