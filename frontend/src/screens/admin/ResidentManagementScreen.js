...
const ResidentManagementScreen = () => {
  ...
  const handleDelete = async (billId) => {
    ...
    if (window.confirm(t("bills.confirmDelete"))) {
      try {
        ...
        const response = await fetchWithAuth(`/api/admin/bills/${billId}`, {
          method: "DELETE",
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showToast(
        editingBill ? t("bills.success.updated") : t("bills.success.created"),
      );
    ...
    <div className="bg-white rounded-lg shadow-lg p-6 md:p-8 text-gray-800">
      ...
      {isFormVisible && (
        <form
          ...
          className="bg-gray-50 p-4 rounded-lg mb-6 space-y-3 border"
        >
          <h3 className="text-lg font-semibold">
            {editingBill ? t("bills.form.editTitle") : t("bills.form.newTitle")}
          </h3>
          <input
            name="household_name"
            value={formData.household_name}
            onChange={handleInputChange}
            placeholder="Unidad (Ej: 101동 1502호)"
          ...
          <input
            ...
            onChange={handleInputChange}
            placeholder="Mes de Facturación (AAAA-MM)"
            className="w-full p-2 border rounded"
            required
            disabled={!!editingBill}
          />
          ...
          <div className="flex gap-2 pt-2">
            ...
            >
              {editingBill ? t("common.saveChanges") : t("common.register")}
            </button>
            <button
              type="button"
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm text-left">
          <thead className="bg-gray-100">
            <tr>
              <th className="p-3">{t("bills.table.unit")}</th>
              <th className="p-3">{t("bills.table.month")}</th>
              <th className="p-3">{t("bills.table.amount")}</th>
              <th className="p-3">{t("bills.table.status")}</th>
              <th className="p-3 text-center">{t("bills.table.actions")}</th>
            </tr>
          </thead>
          <tbody>
            {bills.map((bill) => (
              <tr key={bill.id} className="border-b hover:bg-gray-50">
                ...
                <td className="p-3">
                  <span className={`font-bold ${statusColor[bill.status]}`}>
                    {bill.status}
                  </span>
                </td>
