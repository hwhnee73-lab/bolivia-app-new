...
const FinanceScreen = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const resDel = await fetchWithAuth(
          `/admin/reports/delinquency?from=${fromYm}&to=${toYmVal}`,
        );
  ...
  const filteredIncomes = useMemo(
    () => incomes.filter((x) => withinRange(x.month)),
    [incomes, fromYm, toYmVal],
  );
  const filteredExpenses = useMemo(() => {
    return expenses
      .filter((x) => withinRange(x.month))
      .filter((x) => {
  ...
  const outstandingSum = delinquency.reduce(
  ...
  useEffect(() => {
    try {
      ...
      const imap = filteredIncomes.reduce(
        (m, x) => ((m[x.month] = Number(x.total || 0)), m),
        {},
      );
  ...
  const downloadCsv = () => {
    ...
    filteredIncomes.forEach((r) =>
      lines.push(`${r.month},${Number(r.total || 0)}`),
    );
    lines.push("");
    lines.push('"Expenses"');
    ...
    filteredExpenses.forEach((r) =>
      lines.push(
        `${r.month},"${(r.itemName || r.item_name || "").replace(/"/g, '""')}",${Number(r.total || 0)}`,
      ),
    );
    lines.push("");
    lines.push('"Delinquency"');
    lines.push("month,billed,paid,outstanding,rate");
    delinquency.forEach((r) =>
      lines.push(
        `${r.month},${Number(r.billed || 0)},${Number(r.paid || 0)},${Number(r.outstanding || 0)},${Number(r.rate || 0)}`,
      ),
    );
  ...
  const downloadPdf = () => {
    ...
    if (!w) return;
    const style =
      "<style>table{width:100%;border-collapse:collapse}th,td{border:1px solid #ddd;padding:6px;text-align:right}th{text-align:left;background:#f3f4f6}</style>";
    ...
    const incRows = filteredIncomes
      .map(
        (r) =>
          `<tr><td>${r.month}</td><td>${Number(r.total || 0).toLocaleString()}</td></tr>`,
      )
      .join("");
    const exRows = filteredExpenses
      .map(
        (r) =>
          `<tr><td>${r.month}</td><td style="text-align:left">${r.itemName || r.item_name || ""}</td><td>${Number(r.total || 0).toLocaleString()}</td></tr>`,
      )
      .join("");
    const delRows = delinquency
      .map(
        (r) =>
          `<tr><td>${r.month}</td><td>${Number(r.billed || 0).toLocaleString()}</td><td>${Number(r.paid || 0).toLocaleString()}</td><td>${Number(r.outstanding || 0).toLocaleString()}</td><td>${(Number(r.rate || 0) * 100).toFixed(1)}%</td></tr>`,
      )
      .join("");
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6 md:p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div>
          <label className="block text-sm text-gray-600 mb-1">
            {t("finance.filters.to")}
          </label>
          <input
            type="month"
            value={toYmVal}
            onChange={(e) => setToYmVal(e.target.value)}
            className="w-full p-2 border rounded"
          />
        </div>
        <div>
          <label className="block text-sm text-gray-600 mb-1">
            {t("finance.filters.category")}
          </label>
          <select
            value={category}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white p-4 rounded border">
          <h4 className="font-semibold mb-2">
            {t("finance.sections.incomesMonthly")}
          </h4>
          <table className="w-full text-sm">
            <thead className="bg-gray-100">
              <tr>
                <th className="p-2 text-left">{t("finance.table.month")}</th>
                <th className="p-2 text-right">{t("finance.table.total")}</th>
              </tr>
            </thead>
            <tbody>
        ...
        <div className="bg-white p-4 rounded border">
          <h4 className="font-semibold mb-2">
            {t("finance.sections.expensesMonthItem")}
          </h4>
          <table className="w-full text-sm">
            <thead className="bg-gray-100">
              <tr>
                <th className="p-2 text-left">{t("finance.table.month")}</th>
                <th className="p-2 text-left">{t("finance.table.item")}</th>
                <th className="p-2 text-right">{t("finance.table.total")}</th>
              </tr>
            </thead>
            <tbody>
      ...
      <div className="bg-white p-4 rounded border mt-6">
        <h4 className="font-semibold mb-2">
          {t("finance.sections.delinquencyRange")}
        </h4>
        <table className="w-full text-sm">
          <thead className="bg-gray-100">
            <tr>
              <th className="p-2 text-left">{t("finance.table.month")}</th>
              <th className="p-2 text-right">{t("finance.table.billed")}</th>
              <th className="p-2 text-right">{t("finance.table.paid")}</th>
              <th className="p-2 text-right">
                {t("finance.table.outstanding")}
              </th>
              <th className="p-2 text-right">{t("finance.table.rate")}</th>
            </tr>
          </thead>
          <tbody>
            {delinquency.map((r, idx) => (
              <tr key={idx} className="border-b">
                <td className="p-2">{r.month}</td>
                <td className="p-2 text-right">
                  {Number(r.billed || 0).toLocaleString()}
                </td>
                <td className="p-2 text-right">
