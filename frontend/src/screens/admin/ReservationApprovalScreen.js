...
const ReservationApprovalScreen = () => {
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      ...
      showToast(
        t("reservations.success.marked", { status: newStatus.toLowerCase() }),
      );
    } catch (err) {
  ...
  const statusColor = {
    Aprobada: "text-green-800 bg-green-100",
    Pendiente: "text-yellow-800 bg-yellow-100",
    Rechazada: "text-red-800 bg-red-100",
    Cancelada: "text-gray-800 bg-gray-100",
  };

  if (isLoading)
    return <div className="p-6 text-center">{t("reservations.loading")}</div>;
  if (error)
    return (
          ...
          <tbody>
            {reservations.map((res) => (
              <tr key={res.id} className="border-b hover:bg-gray-50">
                ...
                <td className="p-3">
                  <span
                    className={`px-2 py-1 text-xs rounded-full font-medium ${statusColor[res.status]}`}
                  >
                    {res.status}
                  </span>
                </td>
                <td className="p-3 text-center">
                  {res.status === "Pendiente" && (
                    <div className="flex gap-2 justify-center">
                      <button
                        onClick={() => handleStatusChange(res.id, "Aprobada")}
                        className="text-xs bg-blue-500 text-white px-2 py-1 rounded hover:bg-blue-600"
                      >
                        {t("reservations.actions.approve")}
                      </button>
                      <button
                        onClick={() => handleStatusChange(res.id, "Rechazada")}
