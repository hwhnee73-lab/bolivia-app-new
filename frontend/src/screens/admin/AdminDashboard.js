  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const outstandingSum = delin.reduce(
        ...
        const monthsSet = new Set([
          ...incomes.map((x) => x.month),
          ...expenses.map((x) => x.month),
        ]);
        const months = Array.from(monthsSet).sort();
        const imap = incomes.reduce(
          (m, x) => ((m[x.month] = Number(x.total || 0)), m),
          {},
        );
        const exByMonth = expenses.reduce((m, x) => {
          const key = x.month;
          m[key] = (m[key] || 0) + Number(x.total || 0);
          return m;
        }, {});
    ...
    <>
      <div className="bg-white rounded-lg shadow-lg p-6 md:p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
          <div
            onClick={() => navigateTo("resident_management")}
        ...
        <div className="mt-8">
          <button
            ...
            className="w-full p-3 rounded bg-gradient-to-r from-fuchsia-600 to-violet-500 text-white font-bold"
          >
            {t("communication.aiSummaryButton")}
          </button>
        </div>
      </div>
      <Modal
        ...
        title={t("communication.summaryModalTitle")}
      >
        <div className="text-sm text-gray-700">
          {t("communication.aiDisabled")}
        </div>
      </Modal>
    </>
  );
